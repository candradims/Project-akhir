...
const LaporanSiswaAdmin = () => {
  ...
  const dataBK = [
    ...
    },
    {
      ...
      tanggal: "2025-07-05",
      jenis: "Pribadi",
      isi: "Follow Up Masalah Keluarga",
      tindakan: "Konseling Individual",
      konselor: "Dr. Ahmad Wijaya, S.Pd",
    },
  ];

  // Fungsi filter berdasarkan NIS dan tanggal
  const handleSearch = () => {
    if (!nis) {
      alert("Silakan masukkan NIS siswa");
      return;
    }
    const filteredData = dataBK.filter(
      (bk) => bk.nis === nis && bk.tanggal >= startDate && bk.tanggal <= endDate
    );
  ...
  const handlePrintPDF = () => {
    ...
    const studentName = searchResult[0]?.namaSiswa || "";
    const studentClass = searchResult[0]?.kelas || "";
    ...
    doc.text("LAPORAN BIMBINGAN KONSELING PER SISWA", 148, 15, {
      align: "center",
    });
    ...
    doc.text(
      `NIS: ${nis} | Nama: ${studentName} | Kelas: ${studentClass}`,
      148,
      35,
      { align: "center" }
    );
    ...
    doc.autoTable({
      startY: 65,
      head: [
        ["No", "Tanggal", "Jenis", "Isi Permasalahan", "Tindakan", "Konselor"],
      ],
      ...
      columnStyles: {
        ...
        2: { cellWidth: 25, halign: "center" }, // Jenis
        3: { cellWidth: 65 }, // Isi
        4: { cellWidth: 50 }, // Tindakan
        5: { cellWidth: 45 }, // Konselor
      },
    ...
    // Simpan PDF
    doc.save(`Laporan-Siswa-${nis}-${studentName.replace(/\s+/g, "-")}.pdf`);
  };

  return (
    <>
      <Typography variant="h5" className="fw-bold text-primary mb-3 mt-4">
        Cetak Berdasarkan Siswa
      </Typography>
          ...
          <div className="d-flex justify-content-between align-items-center mt-3">
            <div>
              <small className="text-muted">
                Menampilkan {searchResult.length} layanan untuk siswa{" "}
                {searchResult[0]?.namaSiswa}
              </small>
            </div>
            <Button
              variant="success"
...
export default LaporanSiswaAdmin;
