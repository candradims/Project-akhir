...
const VideoChatKonselor = () => {
  const navigate = useNavigate();

  // Data Konselor Online
  const siswaList = [
    { nis: "9990665125", nama: "Buana Saputra", status: "Online" },
  ];
    ...
    <>
              ...
              <tr key={index}>
                ...
                <td>
                  <Button
                    ...
                    onClick={() =>
                      navigate(`/konselor/videochat/${siswa.nama}`)
                    }
                  >
                    Call
                  </Button>
      ...
      <Paper elevation={3} className="p-3">
        <Table striped bordered hover responsive>
          ...
          <tbody>
            {panggilanMasuk.length > 0 ? (
              panggilanMasuk.map((pesan, index) => (
                <tr key={index}>
                  <td>{pesan.nama}</td>
                  <td>
...
export default VideoChatKonselor;
